//
//  HockeyPlayers.swift
//  TradingCards
//
//  Created by Noah Alexandre Soubliere on 2021-11-09.
//

import SwiftUI

struct HockeyPlayers {
    //MARK: Stored Properties
    let imageName: String
    let title: String
    let navigationSubtitle: String
    let imageCaption: String
    let imageCredit: String
    let bodyText: String
}

let listOfItems = [
    HockeyPlayers(imageName: "RedekBonk", title: "Redek Bonk", navigationSubtitle: "navigationSubtitle", imageCaption: "imageCaption", imageCredit: "imageCredit", bodyText: "bodyText")
    ,
    HockeyPlayers(imageName: "MarkStone", title: "Mark Stone", navigationSubtitle: "navigationSubtitle", imageCaption: "imageCaption", imageCredit: "imageCredit", bodyText: "bodyText")
    ,
    HockeyPlayers(imageName: "BradyTkachuk", title: "Brady Tkachuk", navigationSubtitle: "navigationSubtitle", imageCaption: "imageCaption", imageCredit: "imageCredit", bodyText: "bodyText")
    ,
    HockeyPlayers(imageName: "TimStutzle", title: "Tim Stüzle", navigationSubtitle: "navigationSubtitle", imageCaption: "imageCaption", imageCredit: "imageCredit", bodyText: "bodyText")
]
